...
//
// WHAT THIS ASSERTS, and why it is not a pixel baseline. Two implementation-
// independent facts, either of which the defect broke:
//
...
async function placementIn(scope: Page | Locator): Promise<Placement> {
  const dock = scope.locator(".tipdock");
  const box = dock.locator(".tip");
  await expect(box).toBeVisible();
  return await dock.evaluate((dockEl) => {
...
function expectInsideRow(p: Placement, step: string): void {
...
async function runSequence(
  ...
  const slots = scope.locator(slotSelector);
  ...
  const parked = await placementIn(scope);
  expectInsideRow(parked, `${where} focus day ${focusIndex}`);

  await slots.nth(hoverIndex).hover();
  await expect(slots.nth(hoverIndex)).toHaveClass(/\bon\b/);
  const hovered = await placementIn(scope);
  expectInsideRow(hovered, `${where} hover day ${hoverIndex}`);
  ...
  const restored = await placementIn(scope);
  expectInsideRow(restored, `${where} back on day ${focusIndex}`);
