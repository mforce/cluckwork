...
**Pinned k6 version:** `run-baseline.sh` prefers running k6 through `tools/simulation/k6/shell.nix` (a pinned nixpkgs revision), never a bare `nix-shell -p k6` — and always asserts `k6 version` matches the `EXPECTED_K6_VERSION` recorded at the top of `run-baseline.sh` before starting any rep, failing loudly on a mismatch. This is currently **`k6 v2.1.0`**: `baseline.js`'s inter-phase drain gap relies on VU pool-reuse scheduling behavior confirmed live against `v2.0.0` and re-confirmed against `v2.1.0` (see that file's header) — an unpinned/silently-upgraded k6 could change that behavior without anything else in this harness noticing. See `k6/shell.nix` for the pinned revision and how to deliberately bump it; that revision is currently **stale** (still builds `v2.0.0`) until someone with a nix environment verifies a revision that builds `v2.1.0`.

If `nix-shell` isn't on PATH (no nix installed), `run-baseline.sh` falls back to a bare `k6` on PATH instead of refusing to run — this drops the pinned-nixpkgs-build guarantee (whatever `k6` your package manager gives you is what runs), but `EXPECTED_K6_VERSION` is still enforced either way, so a version drift still fails loudly rather than silently invalidating a run's per-user capacity coverage.
