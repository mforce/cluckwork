...
| KD-3 | Tenancy | **Multi-tenant on one shared database, row-level** | Several farms per self-hosted deployment since #530; SaaS later without a rewrite. Achieved via mandatory `account_id` scoping + global query filter (see §4) |
...
- A self-hosted deployment carries as many account rows as the operator provisions (#530); before that epic it ran one. The code never assumed one, which is why the change was an index swap rather than a rewrite.
- The functional spec models the §4.5 localization settings (timezone, locale, currency, unit system, first day of week, format overrides) on `farms`. There is no farms aggregate yet — `SeedDefaults.FarmId` is a stand-in id — so those columns live on the `accounts` row (#123) — one farm per account, which is why an account is presented to users as a farm. That is where `IFarmClock` already reads the timezone from and where financial rows already copy the currency at creation, so no consumer changes when a real farms table arrives: the settings move, the ports do not.
...
- Tenant strategy is **shared-database/shared-schema**. This is the most SaaS-portable, and it is what #530 activated: several accounts coexist on one deployment, isolated by row. DB-per-tenant is deliberately *not* chosen — it complicates migrations, breaks the single `migrate` verb, and is unnecessary at expected scale. Full reasoning and the accepted costs: [`docs/decisions/530-multi-farm-tenancy.md`](../../docs/decisions/530-multi-farm-tenancy.md).
