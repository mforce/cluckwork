...
public sealed class ExportQueries(AppDbContext db) : IExportQueries
{
    ...
    public async Task<ExportTable?> GetTableAsync(string dataset, CancellationToken ct = default)
        => dataset switch
        {
            "flocks" => Table(
                await db.Flocks.AsNoTracking()
                    .OrderBy(x => x.PlacementDate).ThenBy(x => x.Id).ToListAsync(ct),
            ...
            "bird-movements" => Table(
                await db.BirdMovements.AsNoTracking()
            ...
            "daily-entries" => Table(
                await db.DailyEntries.AsNoTracking()
                    .OrderBy(x => x.Date).ThenBy(x => x.Id).ToListAsync(ct),
            ...
            "daily-entry-grades" => Table(
                await db.DailyEntryGrades.AsNoTracking()
                    .OrderBy(x => x.DailyEntryId).ThenBy(x => x.Id).ToListAsync(ct),
            ...
            "egg-lots" => Table(
                await db.EggLots.AsNoTracking()
                    .OrderBy(x => x.ProductionDate).ThenBy(x => x.Id).ToListAsync(ct),
            ...
            "customers" => Table(
                await db.Customers.AsNoTracking().OrderBy(x => x.Name).ThenBy(x => x.Id).ToListAsync(ct),
                ["id", "name", "phone", "email", "address", "note"],
                x => [x.Id, x.Name, x.Phone, x.Email, x.Address, x.Note]),

            "sales-orders" => Table(
                await db.SalesOrders.AsNoTracking()
                    .OrderBy(x => x.OrderDate).ThenBy(x => x.Id).ToListAsync(ct),
            ...
            "sales-order-items" => Table(
                await db.SalesOrderItems.AsNoTracking()
                    .OrderBy(x => x.SalesOrderId).ThenBy(x => x.Id).ToListAsync(ct),
            ...
            "sales-order-allocations" => Table(
                await db.SalesOrderAllocations.AsNoTracking()
                    .OrderBy(x => x.SalesOrderId).ThenBy(x => x.Id).ToListAsync(ct),
            ...
            "payments" => Table(
                await db.Payments.AsNoTracking()
                    .OrderBy(x => x.PaymentDate).ThenBy(x => x.Id).ToListAsync(ct),
            ...
            "inventory-items" => Table(
                await db.InventoryItems.AsNoTracking().OrderBy(x => x.Name).ThenBy(x => x.Id).ToListAsync(ct),
            ...
            "inventory-lots" => Table(
                await db.InventoryLots.AsNoTracking()
                    .OrderBy(x => x.ReceivedDate).ThenBy(x => x.Id).ToListAsync(ct),
            ...
            "inventory-movements" => Table(
                await db.InventoryMovements.AsNoTracking()
                    .OrderBy(x => x.Date).ThenBy(x => x.CreatedAtUtc).ThenBy(x => x.Id).ToListAsync(ct),
            ...
            "feed-usages" => Table(
                await db.FeedUsages.AsNoTracking()
                    .OrderBy(x => x.Date).ThenBy(x => x.Id).ToListAsync(ct),
            ...
            "water-usages" => Table(
                await db.WaterUsages.AsNoTracking()
                    .OrderBy(x => x.Date).ThenBy(x => x.Id).ToListAsync(ct),
    ...
    private static ExportTable Table<T>(List<T> items, string[] header, Func<T, object?[]> map)
        => new(header, items.Select(map).ToList());
}
