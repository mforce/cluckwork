...
// one is set. Falls back to the app's egg mark when there is no logo, when one
// is still loading, and when it will not load — and, before /account has
// answered at all, to the app's name too.
export function FarmBrand() {
  const { farm } = useFarm();
  const { url } = useLogoObjectUrl(farm?.logoContentHash ?? null);
  ...
  return (
    <span className="brand">
      {url === null ? (
        ...
        // twice.
        <img className="brand-logo" src={url} alt="" />
      )}
