...
describe("ProductsPage create", () => {
  ...
  // #897 review — the Grade select starts at "" (no grade picked yet) with
  // a placeholder OPTION ("Pick a grade…"). MUI's floating label only
  // shrinks out of the way when it reads a non-empty `value`, so without an
  // explicit shrink it sat on top of that placeholder text — visible on the
  // real dialog even though this jsdom render cannot show the overlap
  // itself; the shrink class is the DOM fact that stands in for it.
...
// #703 PR 2 — the abandoned-success hijack (#477 part 2) on Products' create
// dialog. BOTH edit dialogs (product edit, conversion edit) have unreachable
// superseded cases — their reopen triggers are row edit buttons disabled while
// the write is in flight — so each is pinned by a wiring test + mutation, same
// as Customers/Flocks edit. Only create gets abandoned-success behaviour tests.
describe("ProductsPage abandoned-attempt success (#703)", () => {
  function deferred<T>() {
    let resolve!: (v: T) => void;
  ...
  const submitCreate = () => fireEvent.click(within(dialog()).getByRole("button", { name: "Add product" }));
  ...
  it("still refreshes the catalog and rotates the key when an abandoned create succeeds", async () => {
    const gate = deferred<{ id: string }>();
    mockCreate.mockReturnValueOnce(gate.promise as never);
    await renderReady(ADMIN);
    openCreate();
  ...
  it("keeps the create key when the write lands but the refresh fails, so the retry replays it", async () => {
    ...
    mockListProducts.mockResolvedValueOnce([P1, P2]);
    mockListProducts.mockRejectedValueOnce(new ApiError(500, "Server error", "refresh boom"));
    await renderReady(ADMIN);
    openCreate();
    fillCreate("One");
    await act(async () => { submitCreate(); });
  ...
  // Product edit's superseded case is UNREACHABLE (row edit button disabled
  // while busy); pinned by this wiring test + mutation P5, not a behaviour test.
  it("a successful product edit closes its dialog and refreshes the catalog (edit's superseded case is unreachable — see comment)", async () => {
  ...
  // Conversion edit's superseded case is likewise UNREACHABLE; pinned by this
  // wiring test + mutation P6.
