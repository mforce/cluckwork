...
import { DEFAULT_LOCALE, formatCount, formatDate, formatDateShort, formatMoney, formatTime } from "../lib/format";

// §4.5 formatting bound to the farm (#650): one hook, five formatters, so a
...
export function useFormat() {
  ...
  return useMemo(() => ({
    ...
    // #897 review — the farm's own locale, a 2-digit year regardless of the
    // farm's configured `dateFormatOverride`: for the one column dense
    // enough to need the width back (Flocks' Placed), not a general
    // replacement for `date` above.
    dateShort: (isoDate: string) => formatDateShort(isoDate, locale),
