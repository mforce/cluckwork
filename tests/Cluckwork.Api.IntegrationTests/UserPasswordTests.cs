...
public sealed class UserPasswordTests(CluckworkWebApplicationFactory factory)
{
    ...
    public async Task AdminSet_RevokesTheTargetsExistingSessions()
    {
        ...
        Assert.Equal(HttpStatusCode.OK,
            (await factory.CreateClient().PostRefreshAsync(worker.RefreshToken)).StatusCode);

        await admin.PutWithKeyAsync($"/api/v1/users/{id}/password",
            Guid.NewGuid().ToString(), new { newPassword = FreshPassword() });

        // That refresh token is dead: no new access tokens for the old holder.
        Assert.Equal(HttpStatusCode.Unauthorized,
            (await factory.CreateClient().PostRefreshAsync(worker.RefreshToken)).StatusCode);
    }
