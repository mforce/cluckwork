...
// Uses factory.MarkLog()/AssertServerSideCleanAsync() from KestrelLogAssertions —
// extracted there once KestrelRequestBodyLimitTests (#342) became a second
// consumer of the same completion-log polling logic.
    ...
    : IClassFixture<KestrelBackedFactory>
{
    ...
    public async Task NoContentWrite_OverKestrel_CompletesWithoutWritingToTheBody()
    {
        ...
        await factory.AssertServerSideCleanAsync(path, mark, expected: 1);
    }
    ...
    public async Task NoContentWrite_ReplayedOverKestrel_CompletesWithoutWritingToTheBody()
    {
        ...
        await factory.AssertServerSideCleanAsync(path, mark, expected: 2);
    }
    ...
    public async Task BodiedWrite_OverKestrel_StillDeliversBodyOnFirstPassAndReplay()
    {
        ...
        await factory.AssertServerSideCleanAsync("/api/v1/egg-grades", mark, expected: 2);
    }
}
