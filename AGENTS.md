...
- **Migrations + base provisioning (#283):** EF migrations auto-apply on startup (`Database:MigrateOnStartup`, default true — but **Production sets it `false`**; the `migrate` command runs DDL out-of-band, see the #263 bullet below). The default account, the four assignable roles (`Roles.Assignable`), and the default egg grades are **static reference data baked into the migrations themselves** (`migrationBuilder.InsertData`, via EF's `HasData` on their entity configurations) — deterministic and multi-instance-safe by construction, **no runtime seeder, no `Seed:*` config**. `MigrationSecurityReviewTests` asserts no migration ever inserts a credential-shaped row (`AspNetUsers`, `PasswordHash`, `SecurityStamp`, …) — the schema itself is never a place a credential can hide. The first admin is a **separate, deliberately NOT migration-baked** concern — see the #283 bullet below.
