...
- **Every image this repo names is digest-pinned AND scanned (#369).** #267 covers the image we *build*; this covers the ones we *reference*. **Any `image:` in a compose file must be `name:tag@sha256:…`** — both halves: the digest is what makes a pull reproducible and ties running bytes to scanned bytes, the tag is what makes staleness reviewable. That is enforced, not conventional: `.github/scripts/compose-images.mjs` (self-tested, `node --test`) enumerates every third-party pin in `deploy/` and **fails on a ref that is not digest-pinned**, then emits them as a matrix that `ci.yml` Trivy-scans per PR. The failure this closes is worth remembering, because *every* individual piece looked fine: Postgres was digest-pinned, Traefik was pinned to `v3.5` — a **release-branch** tag, which upstream silently stops moving once attention shifts to the next minor — and the Trivy step scanned `image-ref: cluckwork-api:ci`, the application image *only*. So the reference stack sat for eight months on a build with **57 fixable HIGH/CRITICAL** (a CRITICAL Go stdlib CVE among them) while the deploy repo read its production Traefik version out of that same file. **A digest pin is inert by construction** — it never floats to a fix — so pinning without a bump proposer converts "silently drifts" into "silently rots"; Dependabot's **`docker-compose` ecosystem** (a *separate* ecosystem from `docker`, which reads Dockerfiles and never looks at compose files) is what closes that loop. Four things are load-bearing and easy to "simplify" away: (1) the compose render must pass **`--profile '*'`** — `traefik` is behind `profiles: ["prod"]` and a plain render omits it *silently*, so the enumeration succeeds over a smaller set (measured: without the flag it exits 0 listing Postgres alone). That is **guarded**, because a render cannot detect its own omission: each bundle carries the file's **declared** profile list from `docker compose config --profiles` (which reports every profile a file declares regardless of which are enabled), and every declared profile must be carried by some rendered service. An earlier version compared `image:` **counts** between the source text and the render and was unsound — a count cannot tell "one service dropped" from "one dropped and one gained", and the line regex feeding it missed an inline `db: {image: …}`, so an undercount could *cancel* a dropped service (#379 review); (2) the enumerator reads **`docker compose config --format json`**, never raw YAML — same rule as `tools/simulation/verify-harness.sh`: assert the resolved value, and never hand-parse a format the tool that owns it will answer for you; (3) compose files are discovered **by content** (any file under `deploy/` with a top-level `services:` key), never by a `docker-compose*.yml` glob, which silently covers nothing named `compose.yml` or `stack.yml` (#379 review); (4) it refuses an **empty** image set, since a scan job over zero images is green and reads as "all pins clean". **`ignore-unfixed: true` is not sufficient on its own here** — it asks whether the *package* has a fix, but the actionable unit for a pin is the *image*, and those come apart when upstream has not rebuilt (postgres ships a `gosu` built against Go 1.24.6, so 15 HIGH/CRITICAL stdlib findings attach to the newest published image, with nothing to bump to). That gap is handled by **`.github/trivyignore.yaml`** — dated, per-CVE, **path-scoped** exceptions with a *required* `expired_at`, the same escape-hatch contract as `.github/security-exceptions.json`, and deliberately **not** applied to the application image's own scan. Never widen one to a wildcard and never demote the scan to advisory to avoid an entry: verified both directions — the ignore file clears postgres while `traefik:v3.5` still fails red through it.
- **Build-time scanning cannot answer "still clean" (#369).** `ci.yml` runs on `push`/`pull_request`, so a newly published advisory against an unchanged image has **no commit to fire on** — nothing changes, nothing runs, nothing looks. That is the precise mechanism by which the stale Traefik pin survived eight months. `.github/workflows/image-audit.yml` closes it on a **daily** cron (vs. `security-audit.yml`'s weekly, because a base-image CVE is fixed by an upstream rebuild that can land any day): it re-scans the third-party pins *and* the **published application image**, pulled from GHCR rather than rebuilt — a rebuild would take ~20 minutes and scan bytes that have never been deployed, which answers a question nobody asked (#351's premise is that the published digest *is* the artifact). It walks back up to 25 commits to find one with an image, because a `[skip ci]` in a commit message legitimately leaves `main` HEAD unpublished, and it **scans by digest**, not by the `:sha-<commit>` tag it looked the digest up under — that tag is mutable by anyone with `packages: write`, the same reason release promotion refuses to trust it.
