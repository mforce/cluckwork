...
- **Migrations + seed:** EF migrations auto-apply on startup (`Database:MigrateOnStartup`, default true). `DatabaseSeeder` seeds the default account + admin user + `Admin` role on every boot — **credentials only from `Seed:*` config, never a fallback secret**; failures log + skip (don't crash).
...
- **Migrate command + prod migration split (#263):** `dotnet Cluckwork.Api.dll migrate` is a third run-then-exit CLI verb (same shape as `seed`) that applies EF migrations then exits — the **pre-deploy-job entrypoint**. Production sets **`Database:MigrateOnStartup=false`** (`appsettings.Production.json`), so the request-serving process never runs schema DDL; `DatabaseReadyHealthCheck` reports **unhealthy on any pending migration**, so a forgotten migrate job fails the deploy loudly instead of serving a stale schema. `deploy/docker-compose.yml` demonstrates the split — a one-shot `migrate` service runs first, `app` waits on `service_completed_successfully`. This is the app-side of #263's privilege separation; the **remaining host-specific deploy step**: grant the runtime DB role only `USAGE` on the schema + DML/sequence privileges (**no DDL**) and point the migrate job at a separate owner/migrator credential.
