...
- **A change to a write contract must update every non-CI writer in the same PR (#394).** Same failure as #370 with a different trigger: adding a required field, tightening a validator, or changing an aggregate's state machine breaks every automated caller of that endpoint — and the four biggest ones **do not run in CI**, so a fully green PR proves nothing about any of them. They are `SimulationDataSeeder` / `DemoDataSeeder` (the `seed` verb's two profiles), `tools/simulation/k6/` (`bundles.js` POSTs `/api/v1/daily-entries`), and the Playwright specs in `tools/simulation/ui/` (`manager.spec.ts` and `worker.spec.ts` both record and submit) — the E2E suite is `workflow_dispatch` only (#385) and the sim harness is human-started (#370), so the first sign of breakage is a person running `reset.sh` or a baseline, possibly weeks later. **Grep for callers of the endpoint you changed and fix them in the same PR.** The subtle half is that **the fixture itself can become invalid, not just the writers**: a new invariant applies retroactively to rows an earlier seeder already wrote, and `SimulationDataSeeder`'s exact-count validation then fails **closed** (`Failed`), which is correct behaviour that reads exactly like a broken harness. If the seed data no longer satisfies the invariant, updating the seeder is part of the change, not follow-up work. Treat a write-contract PR that leaves these behind like a missing test.
