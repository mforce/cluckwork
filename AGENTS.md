...
- **Suspension is immediate for *use*, not for *issuance* (#579).** Login checks `Account.IsActive` and mints in two steps; a suspension committing between them returns 200 with an inert credential, and the `FOR SHARE` lock that would close the window was declined twice on hot-path cost. #530's "race-safe" means use, not issuance — the inertness rests on four named premises (live middleware read, `RefreshAsync`'s suspended check, suspension's same-transaction epoch/stamp revocation, reactivation's revoke), each pinned by a guard that fails when that premise alone breaks; break any one and #579 reopens. → [`579-suspension-issuance-window.md`](docs/decisions/579-suspension-issuance-window.md)
