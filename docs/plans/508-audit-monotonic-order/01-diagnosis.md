...
| INV-3 | All three provenance round trips stay tenant-scoped by their **own** `AccountId` predicate under `IgnoreQueryFilters`; the predicate is never shared or copied between CTEs. | `AuditEventRepository.GetProvenanceChunkAsync`, `Provenance_LastChange_IsScopedToTheTenant`, `Provenance_MadeOfficial_IsScopedToTheTenant` | 2026-09-05 | front-half signoff |
