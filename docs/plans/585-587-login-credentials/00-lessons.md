...
| Suite counts: before / after / tests added | 1881 (at `06653c6a`) / 1897 (fix commit) / +16 (10 in the fix commit: 5 cache removal-storage, 2 canonical field-clear, 1 no-op raw-text, plus 2 two-axis style tests and 1 contrast pair per theme via `it.each`; the remaining +6 are the pre-existing suite growth from earlier commits on `main` between the two runs) |
