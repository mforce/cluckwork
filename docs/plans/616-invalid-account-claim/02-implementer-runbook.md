...
| Response shape and non-CI SPA title consumers | authenticated external principal with valid `sub` plus missing/malformed `account_id`: pre-fix `CredentialEpochMiddleware` returned 401 `application/problem+json` titled `Auth.CredentialsSuperseded`; post-fix `TenantResolutionMiddleware` returns a bodiless 401. Existing consumers: `web/src/routes/Login.tsx` and `web/src/routes/UsersPage.tsx` inspect that title, but need no code/localization change for this unmintable incomplete-token state. | en/es/tl existing `credentialsSuperseded` copy is no longer selected by this path | 1 | Read `CredentialEpochMiddleware.cs`'s 401 ProblemDetails branch and `Login.tsx`/`UsersPage.tsx` title branches; M10 proves missing/malformed `account_id` rejections stay bare. | Driver recorded the pre-fix credential-epoch ProblemDetails fallback and post-fix bare 401; no SPA/localization code changed. |
