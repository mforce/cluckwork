...
**Ledger total (#611 final race/guard fix):** **35 rows — 1 GREEN control (M0) and 34 RED guard mutations.** M0 is the control and is never causal/RED by design; every other row above is a confirmed-RED guard mutation. The earlier "33 rows" count is stale: MC-ASSIGNMENT-RACE (the write-side lifecycle lookup must use live authorization state, not the request-start flock snapshot — a flock assigned to the caller after the scope snapshot was pinned must still be lifecycle-checked) and MC-ERROR-ORDER (the exception handler must wrap flock-scope resolution so a DB fault there reaches mapped ProblemDetails) were added, for a net 33 + 2 = 35.
