...
**Ledger total (#611 error re-execution fix):** **33 rows — 1 GREEN control (M0) and 32 RED guard mutations.** M0 is the control and is never causal/RED by design; every other row above is a confirmed-RED guard mutation. The earlier "32 rows" count is stale: ME-REEXEC (Codex thread r3878288969, the `/error` re-execution bypass) was added, for a net 32 + 1 = 33.
