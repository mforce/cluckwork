...
| ME-REEXEC | guard, #611 r3878288969 | `FlockScopeResolutionMiddleware.cs`: delete the `if (context.Features.Get<IExceptionHandlerFeature>() is not null) { ... return; }` block | `FlockScopeMiddlewareTests.ErrorReExecution_SkipsAssignmentResolution_WithoutDatabaseAccess` | RED — the resolved-Worker branch reaches the `UserRoleAssignments` query and the unreachable Npgsql connection throws before `next` runs |
| MC-ASSIGNMENT-RACE | guard, #611 final-race-fix | `SubmitDailyEntryHandler.cs`: change the post-guard lifecycle lookup back to `flocks.GetByIdAsync(entry.FlockId, ct)` | `SubmitDailyEntryTests.Submit_AssignmentAddedAfterScopeSnapshot_StillChecksArchivedFlockLifecycle` | RED — the stale request-start scope hides flock B, the lifecycle check is skipped, the live guard passes, and submit succeeds instead of failing with `DailyEntry.FlockNotActive` |
| MC-ERROR-ORDER | guard, #611 final-race-fix | `Program.cs`: move the `app.UseExceptionHandler(...)` block to immediately after `app.UseMiddleware<IdempotencyMiddleware>();` | `CredentialEpochMiddlewareOrderTests.Program_ExceptionHandlerWrapsFlockScopeResolution` | RED — the exception handler no longer wraps `FlockScopeResolutionMiddleware`, so a DB fault raised while resolving flock scope would surface as an unhandled 500 instead of a mapped ProblemDetails response |
| MC-RECORD-FLOCK | guard, #611 sibling-write-race-fix | `RecordDailyEntryHandler.cs`: change the post-guard flock lookup back to `flocks.GetByIdAsync(command.FlockId, ct)` | `FlockScopeTests.ScopedWrites_AssignmentAddedAfterScopeSnapshot_UseLiveFlockAndNaturalKeyState` | RED — the stale request-start scope hides flock B, `Assert.True(dailyResult.IsSuccess)` fails (flock reads as not found) |
| MC-RECORD-NATURAL | guard, #611 sibling-write-race-fix | `RecordDailyEntryHandler.cs`: change the natural-key lookup back to `repository.FindByNaturalKeyAsync(...)` | same fact | RED — the stale-scoped lookup misses flock B's existing entry, so `RecordDailyEntry` creates a second entry for the same natural key and the unique index throws instead of reusing `entryBId` |
| MC-FEED-LINK | guard, #611 sibling-write-race-fix | `RecordFeedUsageHandler.cs`: change the pre-transaction provenance flock lookup back to `flocks.GetByIdAsync(command.FlockId, ct)` | same fact | RED — the stale-scoped lookup returns null for flock B, so the recorded `FeedUsage.DailyEntryId` is null instead of `entryBId` |
| MC-FEED-NATURAL | guard, #611 sibling-write-race-fix | `RecordFeedUsageHandler.cs`: change the provenance natural-key lookup back to `dailyEntries.FindByNaturalKeyAsync(...)` | same fact | RED — the stale-scoped lookup misses flock B's entry, so `FeedUsage.DailyEntryId` is null instead of `entryBId` |
| MC-FEED-LIFECYCLE | guard, #611 sibling-write-race-fix | `RecordFeedUsageHandler.cs`: change the in-transaction flock lookup back to `flocks.GetByIdAsync(command.FlockId, transactionCt)` | same fact | RED — the stale request-start scope hides flock B inside the transaction, `Assert.True(feedResult.IsSuccess)` fails (flock reads as not found instead of the FIFO/lifecycle path running) |
| MC-WATER-FLOCK | guard, #611 sibling-write-race-fix | `RecordWaterUsageHandler.cs`: change the flock lookup back to `flocks.GetByIdAsync(command.FlockId, ct)` | same fact | RED — the stale request-start scope hides flock B, `Assert.True(waterResult.IsSuccess)` fails (flock reads as not found) |
| MC-WATER-PROVENANCE | guard, #611 sibling-write-race-fix | `RecordWaterUsageHandler.cs`: change the provenance natural-key lookup back to `dailyEntries.FindByNaturalKeyAsync(...)` | same fact | RED — the stale-scoped lookup misses flock B's entry, so `WaterUsage.DailyEntryId` is null instead of `entryBId` |
| MT-WRITE-FLOCK-TENANT | guard, #611 final-evidence-fix | `FlockRepository.cs` `GetByIdForFlockScopedWriteAsync`: remove `.Where(f => f.AccountId == accountId)` | `FlockScopeTests.ScopedWriteBypasses_ReinstateTenantIsolation` | RED — a foreign-tenant flock (account B's flock) becomes reachable from account A's context |
| MT-WRITE-NATURAL-TENANT | guard, #611 final-evidence-fix | `DailyEntryRepository.cs` `FindByNaturalKeyForFlockScopedWriteAsync`: remove `e.AccountId == accountId &&` | same fact | RED — a foreign-tenant daily entry (account B's) becomes reachable from account A's context via the natural key |

For each: apply → run the named test → confirm RED → restore → **rebuild** → confirm the full `FlockScopeTests` + `FlockScopeMiddlewareTests` are green again (the two `MG-*`, `MU-UNRESOLVED`, and `ME-REEXEC` SPA/API pairs are verified the same way, against their own test files). Mark each mutant with `// MUTANT M<n>: <what this breaks>` and delete the marker on restore. `grep -rn MUTANT src/ tests/ web/src` must return nothing at the end.
...
**Ledger total (#611 final-evidence-fix):** **44 rows — 1 GREEN control (M0) and 43 RED guard mutations.** M0 is the control and is never causal/RED by design; every other row above is a confirmed-RED guard mutation. The earlier "35 rows" count covered only `SubmitDailyEntryHandler`'s stale-snapshot fix — Codex's final-review FIX-NOW found the same request-start-scope-vs-live-assignment race in the three sibling write handlers (`RecordDailyEntryHandler`, `RecordFeedUsageHandler`, `RecordWaterUsageHandler`), including their natural-key and provenance reads. MC-RECORD-FLOCK, MC-RECORD-NATURAL, MC-FEED-LINK, MC-FEED-NATURAL, MC-FEED-LIFECYCLE, MC-WATER-FLOCK and MC-WATER-PROVENANCE were added for a net 35 + 7 = 42. MT-WRITE-FLOCK-TENANT and MT-WRITE-NATURAL-TENANT were added for the foreign-tenant bypass guard on the two scoped-write lookups, for a net 42 + 2 = 44.

**Submit handler ordering (#611 final race/guard fix):** `SubmitDailyEntryHandler`'s required order is now entry tenant-explicit lookup → live `FlockScopeGuard.CheckAsync` → tenant-explicit flock lifecycle lookup (`GetByIdForFlockScopedWriteAsync`) → lifecycle validation → submit. The lifecycle lookup moved from BEFORE the live guard to AFTER it, and switched from the plain tenant-filtered `flocks.GetByIdAsync` to the same live-authorization-state pattern `DailyEntryRepository.GetByIdForFlockScopedWriteAsync` already used for the entry: running lifecycle validation against the request-start flock scope snapshot let a flock assigned to the caller between the snapshot and the live guard skip its lifecycle check entirely (`MC-ASSIGNMENT-RACE`).

**Sibling write handlers (#611 sibling-write-race-fix):** the same stale-snapshot race applies wherever a handler reads flock or daily-entry state AFTER its live `FlockScopeGuard.CheckAsync` succeeds. `RecordDailyEntryHandler`'s flock lookup and natural-key lookup, `RecordFeedUsageHandler`'s pre-transaction provenance lookup, in-transaction lifecycle lookup, and natural-key lookup, and `RecordWaterUsageHandler`'s flock lookup and natural-key lookup all now use the scoped-write pair `IFlockRepository.GetByIdForFlockScopedWriteAsync` / the new `IDailyEntryRepository.FindByNaturalKeyForFlockScopedWriteAsync` — each bypasses the request-start `FlockScope`-filtered query via `IgnoreQueryFilters()` but reinstates `AccountId` explicitly, so a foreign-tenant id still reads as null while a newly-live sibling assignment sees current state. Only Submit was fixed in the prior round; this statement is now accurate for all four flock-scoped write handlers, not Submit alone.
