...
Both the schema change and the seed change are carried into the hand-maintained
`InitialCreate` migration itself. There is **no follow-on migration** — see the
next paragraph for why one would touch zero rows by construction, and AGENTS.md
(#245) for why adding a dead backfill is the pattern that squash was performed
to eliminate. Cracked and Dirty are therefore saleable in base data on every
database, because a virgin database is the only database. No historical lots
are invented.
