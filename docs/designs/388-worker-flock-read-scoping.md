...
Close the gap where a Worker scoped to one flock can enumerate and read unassigned flocks. The write path already enforces `IFlockScopeGuard` on 4 handlers; the read path consults it on zero. After this slice, a scoped Worker's reads are filtered to their assigned flocks (plus farm-wide rows), and `GET /flocks/{id}` for an unassigned flock returns 404. **Corrected 2026-08-28:** "the write path already enforces `IFlockScopeGuard`" was true at slice start but incomplete by ship — the same slice's review rounds (#611) found and fixed a stale-snapshot race in all 4 write handlers (the live guard passed, but a post-guard read then used the request-start `FlockScope` snapshot instead of live state) and shipped the `GetByIdForFlockScopedWriteAsync`/`FindByNaturalKeyForFlockScopedWriteAsync` fix described in §5.
...
- **Raw-SQL enforcement sites (P1-3, Option A):** Walk every `FromSql*`/`IgnoreQueryFilters` call site on the 8 filtered entities:
  ...
  - `FlockRepository.GetByIdForFlockScopedWriteAsync` / `DailyEntryRepository.FindByNaturalKeyForFlockScopedWriteAsync` (both `IgnoreQueryFilters`) — **write-side, added post-slice (#611 final-race-fix / sibling-write-race-fix), not part of the original read-scoping filter set.** The 4 write handlers (`SubmitDailyEntryHandler`, `RecordDailyEntryHandler`, `RecordFeedUsageHandler`, `RecordWaterUsageHandler`) call `IFlockScopeGuard.CheckAsync` against the request-start `FlockScope` snapshot, then must read the flock/entry's CURRENT (live) state — a plain filtered read would still see the stale snapshot the guard already passed. Both methods bypass the combined tenant+flock query filter via `IgnoreQueryFilters()` for that reason, but reinstate `AccountId` explicitly (`.Where(f => f.AccountId == accountId)` / `e.AccountId == accountId &&`) so the bypass does not also cross tenant boundaries — pinned by `FlockScopeTests.ScopedWriteBypasses_ReinstateTenantIsolation` and its two mutations (`MT-WRITE-FLOCK-TENANT`, `MT-WRITE-NATURAL-TENANT`). Used ONLY after the live `IFlockScopeGuard` check has already succeeded — never as a substitute for it; the guard is the authorization decision, these are the live-state read.
...
- **Concurrency evidence (corrected 2026-08-28: a deterministic cross-transaction mid-request race test now ships, superseding the round-2 "none ships" correction below).** What ships:
  - `SubmitDailyEntryTests.Submit_AssignmentAddedAfterScopeSnapshot_StillChecksArchivedFlockLifecycle` — a request-start `FlockScope` pinned to flock A, a SEPARATE transaction adds a live assignment to flock B and archives it, and Submit is proven to check the flock's CURRENT lifecycle state rather than the stale snapshot.
  - `FlockScopeTests.ScopedWrites_AssignmentAddedAfterScopeSnapshot_UseLiveFlockAndNaturalKeyState` — the same race, driven against the three sibling write handlers (`RecordDailyEntryHandler`, `RecordFeedUsageHandler`, `RecordWaterUsageHandler`): while a live assignment to flock B exists but the handlers' request scope is still pinned to flock A, all three succeed against flock B's live state (including natural-key reuse and feed/water provenance linkage); after flock B is archived in a further separate transaction, a fresh request scope (still pinned to A) rejects all three with the flock's live (not snapshotted) lifecycle error.
  - `Resolve_DifferingScope_ThrowsReassignmentException` separately pins single-assignment/no mid-request scope replacement on `FlockScope` itself (INV-4) — this is a different guarantee (the scope object cannot be silently reassigned) from the race tests above (the write handlers reach live state despite the scope object staying pinned).
  - `RoleMatrixTests.FlockScope_CoversFeed_SubmitOfUnassignedDraft_AndMismatchedUnassign` remains a *separate* next-request test: it removes an assignment and proves the *next* HTTP request observes the changed scope, not a same-request race.
